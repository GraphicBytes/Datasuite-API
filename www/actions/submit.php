...
if ($form_id !== null or $form_id != "") {
    ...
    if ($token_check == 1 && $session_token_check == 1) {
        ...
        // nhs form verification

        if ($form_id == 44) {
            if (isset($_POST['email']) && !empty($_POST['email'])) {
                $nhs_email = $_POST['email'];
                $email_verified = $db->sql("SELECT * FROM ppat_2fa_codes WHERE user_email=? ORDER BY id DESC LIMIT 1", 's', $nhs_email);

                if ($email_verified->num_rows == 0) {
                    $errors = 1;
                    $response = 1;
                    $message = "Email not verified by 2FA";
                    $error_data['email'] = "Email not verified by 2FA";
